...
class Verify {
     ...
     */  
    public static function entry($key="code") {
        self::$seKey=$key;
        // 图片宽(px)  
        ...
        for ($i = 0; $i<self::$length; $i++) { 
			self::$_color = imagecolorallocate(self::$_image, mt_rand(1,120), mt_rand(1,120), mt_rand(1,120)); 
			$ttf = dirname(__FILE__) . '/ttfs/' . mt_rand(1, 2) . '.ttf';
            $code[$i] = self::$codeSet[mt_rand(0, 27)];  
            $codeNX += mt_rand(self::$fontSize*1.2, self::$fontSize*1.6);  
            // 写一个验证码字符  
            imagettftext(self::$_image, self::$fontSize, mt_rand(-35, 35), $codeNX, self::$fontSize*1.5, self::$_color, $ttf, $code[$i]);
        }
     ...
     */  
    public static function check($code,$key="code") {
        self::$seKey=$key;
        isset($_SESSION) || session_start();  
        // 验证码不能为空  
